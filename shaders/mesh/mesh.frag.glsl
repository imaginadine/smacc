...
float stepFunction(float nb, float threshold)
{
    return float(nb > threshold);
}

float getColorStepped(float intensity)
{
    float threshold1 = 0.55;
    float threshold2 = 0.65;
    float threshold3 = 0.75;
	float threshold4 = 0.85;
	float threshold5 = 0.95;

    float result = stepFunction(intensity, threshold1) + stepFunction(intensity, threshold2) + stepFunction(intensity, threshold3) + stepFunction(intensity, threshold4) + stepFunction(intensity, threshold5);
    return result*0.5;
}
